/*
 * WEDDBM: Weighted-Degree EDDBM
 * Improves EDDBM by using weighted degree and local average degree per BFS level.
 */
...
double estimateBOLT(const Graph &G, int v, int T, mt19937 &rng, const vector<double> &wdeg) {
    vector<double> P = weddbmProbs(G, v, wdeg);
...
double measureEfficiency(const Graph &G,
    ...
    for (int i = 0; i < trials; i++) {
        ...
        double estA = estimateBOLT(G, a, T, rng, wdeg);
        double estB = estimateBOLT(G, b, T, rng, wdeg);
...
double measureAvgError(const Graph &G,
    ...
    for (int i = 0; i < sampleNodes; i++) {
        ...
        double est = estimateBOLT(G, v, T, rng, wdeg);
...
int main(int argc, char *argv[]) {
    ...
    string effCsv = base + "_weddbm_efficiency.csv";
    ...
    if (computeExact) {
        for (int t : Tvals) {
            ...
            tr = max(tr, 100);
            double eff = measureEfficiency(G, exactBC, t, tr, rng, wdeg);
    ...
    string errCsv = base + "_weddbm_error.csv";
    ofstream errOut(errCsv);
    errOut << "T,AvgError\n";

    cout << "\n=== Average Error vs T (WEDDBM) ===\n";
    ...
    if (computeExact) {
        ...
        for (int t : Tvals) {
            double err = measureAvgError(G, exactBC, t, sampleN, rng, wdeg);
