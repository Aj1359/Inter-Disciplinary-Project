/*
 * PDEDDBM: Path-Density EDDBM
 * Uses sigma(v,i) from BFS to weight sampling probability.
 */
...
static void bfsSigma(const Graph &G, int v,
                     vector<int> &dist,
                     vector<double> &sigma) {
    int n = G.n;
    dist.assign(n, -1);
...
double estimateBOLT(const Graph &G, int v, int T, mt19937 &rng) {
    vector<double> P = pdeddmProbs(G, v);
...
int main(int argc, char *argv[]) {
    ...
    string effCsv = base + "_pdeddmbm_efficiency.csv";
    ...
    string errCsv = base + "_pdeddmbm_error.csv";
    ofstream errOut(errCsv);
    errOut << "T,AvgError\n";

    cout << "\n=== Average Error vs T (PDEDDBM) ===\n";
