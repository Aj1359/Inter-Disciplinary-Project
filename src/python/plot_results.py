...
if prob_frames:
    ...
    for i, label in enumerate(labels):
        df = prob_frames[label]
        x = range(len(df))
        axs[i].plot(x, df["EDDBM"], label="EDDBM", color=COLORS[0], linewidth=1.6)
        axs[i].plot(x, df["Optimal"], label="Optimal", color=COLORS[1], linewidth=1.6)
        axs[i].set_title(label, fontsize=11)
        axs[i].set_xlabel("Random Node Sample", fontsize=9)
        axs[i].set_ylabel("Assigned Probability", fontsize=9)
        axs[i].grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig("bolt_prob_compare_line.png", dpi=150, bbox_inches="tight")
    print("Saved: bolt_prob_compare_line.png")
    plt.show()
