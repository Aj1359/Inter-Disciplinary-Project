...
int main(int argc, char *argv[]) {
  string filename = "datasets/synthetic.txt";
  int target = -1;
  int topk = 50;
  bool runBrute = false;
  bool compare = false;

  // We will test 4 values of tau
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (a == "--brute")
      ...
      target = stoi(argv[++i]);
    else if (a[0] != '-' && filename == "datasets/synthetic.txt")
      filename = a;
    else if (a[0] != '-')
      target = stoi(a);
  }
  ...
  if (target < 0) {
    ...
    target = max_element(bc.begin(), bc.end()) - bc.begin();
    printf("\nMax betweenness node: %d  (Value = %.6f)\n", target, bc[target]);
  } else {
    printf("\nTarget node: %d\n", target);
  }
  ...
  if (compare || runBrute) {
    ...
    printResult(rBrute, "Brute Force (Global Maximum)");
  }
  ...
  for (double tau : taus) {
    ...
    if (compare || !runBrute) {
      rSmart = smartAlgorithm(G, target, tau, topk);
      printResult(rSmart, "Smart Algo");
    }
  }
